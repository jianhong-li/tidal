...
import tidal.remoting.common.SemaphoreReleaseOnlyOnce;
...
public abstract class NettyRemotingAbstract {
	 ...
	 */
	private static final Logger PLOG = LoggerFactory.getLogger(LoggerName.REMOTING_LOGGER_NAME);
	...
	public void processRequestCommand(ChannelHandlerContext ctx, RemotingCommand cmd) {
		...
		if (matched != null) {
			Runnable run = new Runnable() {
				...
				public void run() {
					try {
						final RemotingCommand response = matched.getObject1().processRequest(ctx, cmd);
			...
			if (matched.getObject1().rejectRequest()) {
				final RemotingCommand response = RemotingCommand.createResponseCommand(
			...
			try {
				...
				this.getCallbackExecutor().submit(requestTask);
			} catch (RejectedExecutionException e) {
				if ((System.currentTimeMillis() % 10000) == 0) {
					PLOG.warn(RemotingHelper.parseChannelRemoteAddr(ctx.channel()) //
							+ ", too many requests and system thread pool busy, RejectedExecutionException " //
							+ matched.getObject2().toString() //
